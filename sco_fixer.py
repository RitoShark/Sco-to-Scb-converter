...
def convert_sco_bytes_to_scb(sco_data):
    ...
    # The old SCO engine ALWAYS subtracted CentralPoint from vertices.
    # SCB uses vertices as-is. To match the old behavior (so bin EmitterPosition
    # values etc. still work correctly), we must subtract CentralPoint ourselves.
    if so.central is not None and (so.central.x != 0.0 or so.central.y != 0.0 or so.central.z != 0.0):
        cx, cy, cz = so.central.x, so.central.y, so.central.z
        for pos in so.positions:
            pos.x -= cx
            pos.y -= cy
            pos.z -= cz
        if so.pivot is not None:
            so.pivot.x -= cx
            so.pivot.y -= cy
            so.pivot.z -= cz
        so.central.x = 0.0
    ...
    # Set defaults for SCB - Riot uses flags=5 (bits 0+2) for SCB files
    if so.flags is None:
        so.flags = 5  # Riot's standard SCB flag value
